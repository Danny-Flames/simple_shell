...
/**
 * repeated_char - To count the repetitions of a char
 *
 * @input: input string
 * @ind: index
 * Return: repetitions
...
/**
 * error_sep_op - search for syntax errors
 *
 * @input: input string
 * @ind: index
 * @last: last char read
...
int check_syntax_error(data_shell *datash, char *input)
{
	...
	ind = error_sep_op(input + begin, 0, *(input + begin));
	if (ind != 0)
	{
		print_syntax_error(datash, input, begin + ind, 1);
		return (1);
	}
